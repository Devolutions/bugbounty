...
def _require_privileges(script_args: list[str]) -> None:
    ...
    if IS_WINDOWS:
        ...
        if os.geteuid() != 0:
            ...
            os.execvp("sudo", ["sudo", sys.executable] + script_args)
            # execvp replaces the current process — code below is unreachable
...
def _build_env(script_dir: Path) -> None:
    ...
    if env_path.exists():
        ...
        env_path.unlink()
        print("🧹 Removed existing .env")

    # Copy template, normalise line endings
    ...
    # Append certificate placeholder variables
    placeholders = (
...
def _inject_certificates(env_path: Path, cert_dir: Path) -> None:
    """Base64-encode all certificate files and inject them into .env."""
    mappings = {
...
def _cert_state(cert_dir: Path) -> dict[str, bool]:
    """Return which cert groups are fully present."""
...
def _install_ca_linux_rhel(ca_cert: Path) -> None:
    install_path = Path("/etc/pki/ca-trust/source/anchors/devolutions-ca.crt")
...
def _install_ca(ca_cert: Path) -> None:
    """Install CA into the OS trust store (and browser stores on Linux)."""
    if IS_WINDOWS:
...
def _configure_firefox_linux(ca_cert: Path) -> None:
    """Write Firefox enterprise policy so it auto-trusts the CA."""
...
def _ensure_hosts_entry(hostname: str) -> None:
    ...
    # Match lines like "127.0.0.1  gateway.loc" (with optional extra entries)
...
def main() -> None:
    ...
    parser.add_argument("--skip-ca-validation", action="store_true",
                        help="Skip CA certificate installation")
    parser.add_argument("--clean", action="store_true",
                        help="Stop containers and wipe data folders before starting")
    parser.add_argument("--update", action="store_true",
                        help="Pull latest container images before starting")
    parser.add_argument("--no-cert-gen", action="store_true",
                        help="Use existing certificates only, do not generate new ones")
    args = parser.parse_args()

    _require_privileges(sys.argv)
    ...
    if args.update:
        ...
        if result.returncode != 0:
            print("Error: Failed to update containers.")
            sys.exit(1)
    ...
    # Certificate generation / reuse logic
    if args.no_cert_gen:
    ...
    if not args.skip_ca_validation:
        ...
        print("⚠️  Skipping CA certificate validation (--skip-ca-validation flag set)")
    ...
    if os_type != "linux":
        print(f"Error: Docker is not running in Linux Containers mode. (Detected: {os_type})")
        sys.exit(1)
    ...
    # Sync Gateway certificate thumbprint in DB
    sql_password = env.get("SQL_MSSQL_PASSWORD", "")
    _sync_gateway_thumbprint(cert_dir, sql_password)
